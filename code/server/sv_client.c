...
void MOD_AddHealth(client_t *cl, int value) {
    ...
    if (ent->health <= 0 || ps->persistant[PERS_TEAM] == TEAM_SPECTATOR || *(int *) (
            (void *) ps + gclientOffsets[getVersion()][OFFSET_TEAM]) == TEAM_SPECTATOR)
        return;
...
static void SV_LoadPosition_f(client_t *cl) {
    ...
    // regenerate stamina
    ps->stats[playerStatsOffsets[getVersion()][OFFSET_PS_STAMINA]] =
            ps->stats[playerStatsOffsets[getVersion()][OFFSET_PS_HEALTH]] * 300;
...
void SV_ServerModInfo_f(client_t *cl) {
    ...
    SV_SendServerCommand(cl, "chat \"%s^7Game Version: ^1Urban Terror %s\"", sv_tellprefix->string,
                         Cvar_VariableString("g_modversion"));
    SV_SendServerCommand(cl, "chat \"^2==============================================================\"");
    SV_SendServerCommand(cl, "chat \"%s^7Credits: ^3Titan Mod ^7was developed by ^5Pedrxd ^7& ^5Th3K1ll3r\"",
                         sv_tellprefix->string);
    SV_SendServerCommand(cl, "chat \"^2==============================================================\"");
}
...
void SV_HelpCmdsList_f(client_t *cl) {
    ...
    if (version == jump) {
        ...
        SV_SendServerCommand(
            cl, "print  \"^8-^7--------> Help Client - ^6List of Commands ^7[Jump Mode] <--------^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(cl, "print  \"^8------------------------^7General Commands^8------------------------\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-    ^1/help           ^8-     ^2/playerlist     ^8-    ^2/cg_rgb        ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-    ^2/reconnect      ^8-                     ^8-    ^2/r_gamma       ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-    ^2/disconnect     ^8-     ^1/svModInfo      ^8-    ^2/sensitivity   ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-    ^2/connect        ^8-                     ^8-    ^2/bind          ^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(cl, "print  \"^8--------------------------^7Game Commands^8-------------------------\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-         ^2$location             ^8-       ^2/regainstamina         ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-         ^2/name                 ^8-       ^2/save                  ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-         ^2/kill                 ^8-       ^2/load                  ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-         ^2/ready ^5(RUN)          ^8-       ^2/allowgoto             ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-         ^2/cg_ghost ^5<0|1>       ^8-       ^2/goto                  ^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/infiniteStamina       ^7[Turn ON/OFF Infinite Stamina]        ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/infiniteWallJumps     ^7[Turn ON/OFF Infinite Wall Jumps]     ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/hidePlayers           ^7[Make all other players invisible]    ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/freeSave              ^7[Turn ON/OFF the free position saving]^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/tell ^5<client> <message>                  ^7[Private Messages] ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/callvote ^5nextmap <mapname> ^7or ^5cyclemap   ^7[Map Votes]        ^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        ...
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-^7--------> Help Client - ^6List of Commands ^7[Frag Mode] <--------^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(cl, "print  \"^8------------------------^7General Commands^8------------------------\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
        ...
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-           ^2/name            ^8-            ^2$location            ^8-\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8-           ^2/kill            ^8-                                 ^8-\n\"");
        SV_SendServerCommand(cl, "print  \"^8----------------------------------------------------------------\n\"");
        SV_SendServerCommand(
            cl, "print  \"^8- ^2/tell ^5<client> <message>                  ^7[Private Messages] ^8-\n\"");
        SV_SendServerCommand(
...
char *badwords[43] = {
    "fuck", "dick", "shit", "slut", "hure", "cunt", "homo", "puta", "nazi", "fick", "fotze", "opfer", "arsch",
    ...
    if (clientOK) {
        ...
        if ((!u->name) && (sv.state == SS_GAME) && (cl->state == CS_ACTIVE)) {
            ...
            if (Q_stricmp("say", Cmd_Argv(0)) == 0) {
                ...
                if (mod_customchat->integer && sv_gametype->integer == GT_FFA) {
                    if (!cl->muted) {
                        ...
                        SV_SendServerCommand(NULL, "chat \"%s^7%s^3:^%i %s\"", getNewPrefixFFA(cl, ps), cl->name,
                                             cl->chatcolour, message);
                    }
                    ...
                    return;
                           ...
                           mod_zombiemod->integer) {
                    if (!cl->muted) {
                        // Send the chat message
                        SV_SendServerCommand(NULL, "chat \"%s^7[^2%i^7]^3%s:^%i %s\"", getZombiePrefix(ps), cl->level,
                                             cl->name, getColourPrefix(cl), message);
                    }
                    SV_LogPrintf("say: %i %s: %s\n", ps->clientNum, cl->name, CopyString(Cmd_Args()));
                    return;
                } else if (mod_customchat->integer && sv_gametype->integer == GT_SURVIVOR && mod_levelsystem->integer) {
                    ...
                    if (!cl->muted) {
                        // Send the chat message
                        SV_SendServerCommand(NULL, "chat \"%s^7[^2%i^7]^3%s:^%i %s\"", getNewPrefixSR8(ps), cl->level,
                                             cl->name, getColourPrefix(cl), message);
                    }
                    SV_LogPrintf("say: %i %s: %s\n", ps->clientNum, cl->name, CopyString(Cmd_Args()));
                    return;
                    ...
                    if (!cl->muted) {
                        ...
                        if (team != 3) {
                            // Send the chat message
                            SV_SendServerCommand(NULL, "chat \"^7[^3Arena ^5%i^7][^3SP:^5%i^7]^3%s: %s\"",
                                                 cl->arena + 1, cl->skill, cl->colourName, message);
                        } else {
                            SV_SendServerCommand(NULL, "chat \"^7[^5No Arena^7][^3SP:^5%i^7]^3%s: %s\"", cl->skill,
                                                 cl->colourName, message);
                        }
                    }
            ...
            */
            ...
            if (Q_stricmp("say", Cmd_Argv(0)) == 0 && sv_gametype->integer != GT_JUMP && SV_GetClientTeam(cid) ==
                TEAM_SPECTATOR && mod_specChatGlobal->integer) {
                argsFromOneMaxlen = MAX_SAY_STRLEN;
                ...
                return;
            } else if (Q_stricmp("ut_itemdrop", Cmd_Argv(0)) == 0 && (
                           !Q_stricmp("flag", Cmd_Argv(1)) || !Q_stricmp("1", Cmd_Argv(1)) || !
                           Q_stricmp("2", Cmd_Argv(1))) && !mod_allowFlagDrop->integer) {
                ...
                return;
            } else if (Q_stricmp("kill", Cmd_Argv(0)) == 0 && ps->stats[playerStatsOffsets[getVersion()][
                           OFFSET_PS_HEALTH]] < mod_minKillHealth->integer) {
                SV_SendServerCommand(
                    cl, "print \"^1You need a minimum of ^2%i percent ^1of health to kill yourself!\n\"",
                    mod_minKillHealth->integer);
                return;
            } else if (Q_stricmp("team", Cmd_Argv(0)) == 0 && ps->stats[playerStatsOffsets[getVersion()][
                           OFFSET_PS_HEALTH]] < mod_minTeamChangeHealth->integer && SV_GetClientTeam(cid) !=
                       TEAM_SPECTATOR) {
                SV_SendServerCommand(
                    cl, "print \"^1You need a minimum of ^2%i percent ^1of health to change of team!\n\"",
                ...
                switch (mod_allowTeamSelection->integer) {
                    ...
                    case 2:
                        if (Q_stricmp("s", Cmd_Argv(1)) != 0 && Q_stricmp("free", Cmd_Argv(1)) != 0) {
                            SV_SendServerCommand(
                                cl, "print \"^1You can only spectate or auto join in this server!\n\"");
                            return;
                        }
                        break;
                    case 3: {
                        ...
                        if ((Q_stricmp("free", Cmd_Argv(1)) == 0)) {
                            if (team == TEAM_SPECTATOR) {
                                ...
                                SV_SendServerCommand(
                                    cl, "print \"^1Only spectators can do auto join in this server!\n\"");
                                return;
                            }
                        }
...
void MOD_AutoHealth(client_t *cl) {
    ...
    if (cl->cm.lastAutoHealth < svs.time) {
        ...
        if (ent->health < (cl->cm.perPlayerHealth ? cl->cm.limitHealth : mod_limitHealth->integer)) {
            if ((cl->cm.perPlayerHealth ? !cl->cm.whenmovingHealth : !mod_whenMoveHealth->integer) &&
                SV_ClientIsMoving(cl))
                return;
...
void giveMultiClientWeapon(client_t *client, client_t *client2) {
    ...
    if (usePreference == ARENA_PRIMARY) {
        ...
        SV_SendServerCommand(
            client, "cchat \"\" \"^5[^3Arena^5] ^1Error: [GiveMultiWeapon Failed] ^3contact ^2donna30.\"");
        SV_SendServerCommand(
            client2, "cchat \"\" \"^5[^3Arena^5] ^1Error: [GiveMultiWeapon Failed] ^3contact ^2donna30.\"");
        return;
    }
    ...
    if ((time - sv.doTeleportTime) > 10000) {
        ...
        for (int i = 0; i < MAX_1V1_ARENAS; i++) {
            if (sv.Arenas[i].population == 2) {
                ...
                ps->stats[playerStatsOffsets[getVersion()][OFFSET_PS_STAMINA]] =
                        ps->stats[playerStatsOffsets[getVersion()][OFFSET_PS_HEALTH]] * 300;
                ps2->stats[playerStatsOffsets[getVersion()][OFFSET_PS_STAMINA]] =
                        ps2->stats[playerStatsOffsets[getVersion()][OFFSET_PS_HEALTH]] * 300;
            }
        }
        sv.doneTp = qtrue;
    }
