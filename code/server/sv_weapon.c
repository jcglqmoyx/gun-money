    ...
    switch (itemid) {
        case UT_ITEM_AMMO: {
            for (i = 0; i < MAX_POWERUPS; i++) //UT_MAX_WEAPONS
            {
                ...
                if ((weaponID == UT_WP_GRENADE_HE) || (weaponID == UT_WP_GRENADE_FLASH) || (
                        weaponID == UT_WP_GRENADE_SMOKE)) {
                    continue;
                }
    ...
    for (weapon = UT_WP_KNIFE; weapon < (UT_WP_NUM_WEAPONS + 3); weapon++) {
        ...
        for (i = 0; i < HL_MAX; i++) {
            if (Q_stricmp(Cvar_VariableString(va("%s_damages_%s", weaponString[weapon], hitlocationstring[i])), "")) {
                *(float *) QVM_damages(weapon, i) = Cvar_VariableValue(
                    va("%s_damages_%s", weaponString[weapon], hitlocationstring[i]));
            }
        ...
        for (i = 0; i < 3; i++) {
            if (Q_stricmp(Cvar_VariableString(va("%s_%i_firetime", weaponString[weapon], i)), "")) {
                *(int *) QVM_fireTime(weapon, i) = Cvar_VariableIntegerValue(
                    va("%s_%i_firetime", weaponString[weapon], i));
            }
