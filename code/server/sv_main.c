...
void QDECL SV_SendServerCommand(client_t *cl, const char *fmt, ...) {
	...
	if ((Q_strncmp((char *) message, "location", 8) == 0) && (sv_gametype->integer == GT_JUMP) && (mod_jumpSpecAnnouncer
		    ->integer)) {
...
qboolean SV_CheckDRDoS(netadr_t from) {
	...
	if (globalCount == MAX_INFO_RECEIPTS) {
		// All receipts happened in last 2 seconds.
		if (lastGlobalLogTime + 1000 <= svs.time) {
	...
	if (specificCount >= 3) {
		// Already sent 3 to this IP in last 2 seconds.
		if (lastSpecificLogTime + 1000 <= svs.time) {
			// Limit one log every second.
			Com_DPrintf(
				"Possible DRDoS attack to address %i.%i.%i.%i, ignoring getinfo/getstatus connectionless packet\n",
